 ...
 * 
 */
public abstract class CaGeneFilterServiceResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CaGeneFilterServiceResourceBase.class);

	private CaGeneFilterServiceResourceConfiguration configuration;
    ...
    public CaGeneFilterServiceResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CaGeneFilterServiceResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CaGeneFilterServiceConstants.SERVICEMETADATA);
	...
	public CaGeneFilterServiceResourceConfiguration getConfiguration() {
		...
		servicePath+="/CaGeneFilterService";
		...
		try {
			...
			this.configuration = (CaGeneFilterServiceResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CaGeneFilterServiceResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
