...
import org.bioconductor.packages.caPROcess.common.CaPROcessConstants;
import org.bioconductor.packages.caPROcess.stubs.CaPROcessResourceProperties;
import org.bioconductor.packages.caPROcess.service.CaPROcessConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class CaPROcessResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CaPROcessResourceBase.class);

	private CaPROcessResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CaPROcessResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CaPROcessConstants.SERVICEMETADATA);
	...
	public CaPROcessResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (CaPROcessResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CaPROcessResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
