...
import org.bioconductor.packages.helper.common.HelperServiceConstants;
import org.bioconductor.packages.helper.stubs.HelperServiceResourceProperties;
 ...
 * 
 */
public abstract class HelperServiceResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(HelperServiceResourceBase.class);

	private HelperServiceResourceConfiguration configuration;
    ...
    public HelperServiceResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((HelperServiceResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(HelperServiceConstants.SERVICEMETADATA);
	...
	public HelperServiceResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (HelperServiceResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((HelperServiceResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
