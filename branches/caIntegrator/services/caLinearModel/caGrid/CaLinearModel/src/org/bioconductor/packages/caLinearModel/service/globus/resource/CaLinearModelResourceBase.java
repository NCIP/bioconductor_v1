package org.bioconductor.packages.caLinearModel.service.globus.resource;
...
import org.bioconductor.packages.caLinearModel.common.CaLinearModelConstants;
import org.bioconductor.packages.caLinearModel.stubs.CaLinearModelResourceProperties;
import org.bioconductor.packages.caLinearModel.service.CaLinearModelConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class CaLinearModelResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CaLinearModelResourceBase.class);

	private CaLinearModelResourceConfiguration configuration;
    ...
    public CaLinearModelResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CaLinearModelResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CaLinearModelConstants.SERVICEMETADATA);
	...
	public CaLinearModelResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (CaLinearModelResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CaLinearModelResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
