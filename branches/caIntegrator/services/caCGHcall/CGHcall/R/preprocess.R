...
#        }
    ...
    } else if (method == "DNAcopyNumberedSegments") {
    
        CNA.object  <- DNAcopy::CNA(copynumber(input), chromosomes(input), bpstart(input), data.type="logratio", sampleid=sampleNames(input))
        cat("Start data segmentation .. \n")
        segmented  <- DNAcopy::segment(CNA.object, ...)
        ...
        numclone    <- segmented$output$num.mark
        smrat       <- segmented$output$seg
        starts      <- segmented$output$loc.start
        numsmrat    <- cbind(smrat, 1:length(smrat), numclone)
        repdata     <- function(row) {
            rep(row[1:2], row[3])
        }
        makelist    <- apply(numsmrat, 1, repdata)
        joined      <- unlist(makelist) #changed 23/6/09, much faster
        rm(makelist)
        joined      <- matrix(joined, ncol=ncol(input)+1,byrow=TRUE)
        joined      <- CGHcall:::.assignNames(joined, input, "Segment Number")
        result      <- CGHcall:::.segFromRaw(input, as.matrix(joined[,1]), segnumber=as.matrix(joined[,2]))
    }
