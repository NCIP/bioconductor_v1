...
#    inputSegmented <- seg; prior="auto"; nclass=4; organism="human"; robustsig="yes";nsegfit=500;maxnumseg=100;minlsforfit=0.5
  ...
  #  datareg     <- CGHcall:::.MakeData(data.frame(featureNames(inputSegmented), chromosomes(inputSegmented), bpstart(inputSegmented), copynumber(inputSegmented), segmented(inputSegmented)))  
    ...
    nc    <- ncol(datmat)/2 
    nclone <-nrow(datmat)
    # CAP - Add as.matrix() below to handle single column results
    datareg  <- .MakeData(as.matrix(datmat[,-(1:nc)]),chr)  
...
ExpandCGHcall <- function(listcall,inputSegmented, digits=3,divide=4, memeff = FALSE, fileoutpre="Callobj_"){
  ...
  for(part in 1:divide){
   ...
   #     genomdat        <- dataprob[,(ncpp*(i-1)+1)] #removed 16/7/10
    ...
    if (nclass == 4) {assayData <-assayDataNew(copynumber=copynumber(IS),segmented=segmented(IS),calls=calls,probloss=probloss,probnorm=probnorm,probgain=probgain,probamp=probamp,segnumber=Biobase:::assayDataElement(IS, "segnumber"))} else {
    assayData <-assayDataNew(copynumber=copynumber(IS),segmented=segmented(IS),calls=calls,probloss=probloss,probnorm=probnorm,probgain=probgain,segnumber=Biobase:::assayDataElement(IS, "segnumber")) }
