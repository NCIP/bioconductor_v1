...
import org.bioconductor.packages.caDNAcopy.common.CaDNAcopyConstants;
import org.bioconductor.packages.caDNAcopy.stubs.CaDNAcopyResourceProperties;
import org.bioconductor.packages.caDNAcopy.service.CaDNAcopyConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class CaDNAcopyResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CaDNAcopyResourceBase.class);

	private CaDNAcopyResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CaDNAcopyResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CaDNAcopyConstants.SERVICEMETADATA);
	...
	public CaDNAcopyResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (CaDNAcopyResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CaDNAcopyResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
