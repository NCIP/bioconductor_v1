    ...
    function(targets)
{
    ...
    ## basic work flow -- read files, background correct, normalize within
    ## arrays, fit a simple design, 'empirical Bayes' to make estimates more
    ## robust, output 'topTable' with p.values adjusted for false discovery.
...
{
    ## TODO: Test
    ## targets <- readTargets("MUTaa_WILD_rem_mod_target_file") the
    ## targets are the same files that are found in:
    ...
    RG <- read.maimages(targets,
                        columns = list(G="gTotalGeneSignal",
                          Gb="gProcessedSignal"),
                        annotation = c("FeatureNum", "ControlType",
                          "ProbeName", "GeneName"),
                        other.columns = list(
                          gIsGeneDetected = "gIsGeneDetected"),
                        green.only=TRUE)
    RG.sel <- RG[!duplicated(RG$genes$GeneName),]
    RG.sel$E <- RG.sel$E + 23
    ma <- normalizeBetweenArrays(RG.sel, method="quantile")
    dimnames(ma) <-
        list(RG.sel$genes$ProbeName,
             sub("\\?file.*", "", basename(targets[["FileName"]])))
    ma <- ma[ma$gene$ControlType==0,]
    miRs <- c(grep("hsa-miR-", ma$genes$GeneName),
              grep("hsa-let-", ma$genes$GeneName))
    ma <-ma[miRs, ]
