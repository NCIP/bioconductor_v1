TopTableSingleTissueSingleMutgene <-
    function(miEset, input.tissue, mut.gene, outDir=".")
{
    QVAL_CUTOFF = 0.2

    ## make a MUT_WILD and an ERROR subdir
    if(!file.exists('./MUT_WILD'))
        dir.create(paste(outDir,"/MUT_WILD",sep=""))
    if(!file.exists('./ERROR'))
        dir.create(paste(outDir,"/ERROR",sep=""))
    ...
    fData(miEsetHsTisSp)$mir <-
        with(fData(miEsetHsTisSp), 
             paste(Description, "_", Accession, sep = ""))
    ...
    ## At least expressed in one cell line
    miEsetHsTisSp_filter <-
        miEsetHsTisSp[fData(miEsetHsTisSp)$max >= 5 &
    ...
    if( length(rownames(psel.tissue)) > 7 && length(levels(f)) > 1 ){ 
        ...
        if( length(rownames(dw)) > 3 && length(rownames(dm)) > 3 ){
            ...
            contrast.matrix <-
                makeContrasts(contrasts = matString, levels=design)
            ...
            down_in_mut <-
                subset(output,faa.mut.fwt < 0 & adj.P.Val < QVAL_CUTOFF &
                       abs(faa.mut.fwt) > 1)
            up_in_mut <-
                subset(output,faa.mut.fwt > 0 & adj.P.Val < QVAL_CUTOFF &
                       abs(faa.mut.fwt) > 1 )
